...
resource "azurerm_container_app" "eportal" {
  name                            = "${var.name_prefix}-ca-eportal"
  ...
  template {
    container {
      name   = "${var.name_prefix}-c-eportal"
      image  = "${var.ghcr_url}/vgdagpin/aerish.eportal:${var.aerish_version_eportal}"
      cpu    = "0.5"
      memory = "1Gi"
      args   = []
      command = []
...
output "EPortal_fqdn" {
  value = azurerm_container_app.eportal.ingress[0].fqdn
}
